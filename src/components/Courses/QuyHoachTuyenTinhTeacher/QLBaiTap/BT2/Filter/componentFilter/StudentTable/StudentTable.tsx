...
interface StudentTableProps {
  ...
  handleGradeAssignment: (email: string, assignmentKey: "BT1") => void;
}

const StudentTable: React.FC<StudentTableProps> = ({ students, fileData, openModal, handleGradeAssignment }) => (
    ...
    <tbody>
      {students.map((student, index) => (
        <tr key={index}>
          ...
          <td>{student.BT1 === -1 ? "Chưa chấm" : student.BT1}</td>
          <td>
            <button
              className="grade-button"
              onClick={() => handleGradeAssignment(student.email, "BT1")}
