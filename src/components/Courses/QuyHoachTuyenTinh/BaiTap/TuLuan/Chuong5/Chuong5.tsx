...
const Chuong5 = ({ setSelectedItem }: { setSelectedItem: (value: string | null) => void }) => {
    ...
    const [user, setUser] = useState<{ username: string; name: string; phone: string; sex: string; mssv: string; class: string; BT5: number } | null>(null);
...
>
  Turn in
            ...
            <h2>Bài tập Chương 4 Phần 2</h2>

            {/* Hiển thị thông tin về file đề bài */}
            <AssignmentFile 
    fileName={"Baitaptuan4.2.pdf"} 
    fileUrl={"/documents/Baitap/Baitaptuan4.2.pdf"} 
/>
    ...
    <div className="grade-info">
        <h3>Điểm bài tập</h3>
        <p><strong>Điểm:</strong> {user.BT5 === -1 ? "Chưa chấm" : user.BT5}</p>
    </div>
) }
