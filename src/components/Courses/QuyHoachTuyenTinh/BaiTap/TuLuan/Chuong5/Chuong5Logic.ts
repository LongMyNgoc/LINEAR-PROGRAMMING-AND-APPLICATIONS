...
export const useChuong5Logic = () => {
    const [file, setFile] = useState<File | null>(null);
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [fileData, setFileData] = useState<{ fileUrl: string; fileName: string; submissionDate: string } | null>(null);

    const fileUrl = "/documents/Baitap/Baitaptuan4.2.pdf";
    const fileType = "pdf";
    const fileName = "Baitaptuan5.pdf";

    const fetchFileData = async (mssv: string) => {
        try {
            const data = await getFileData(mssv, "TuLuanChuong5");
            setFileData(data);
        } catch (error) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            await submitFile(file, "TuLuanChuong5");
