...
import { useTN3Quiz } from "./useTN3Quiz";

const TN3 = ({ setSelectedItem }: { setSelectedItem: (value: string | null) => void }) => {
  const storedUser = localStorage.getItem("user");
  const userEmail = storedUser ? JSON.parse(storedUser).email : null;

  const { student, answers, handleAnswerChange, calculateScore, handleSubmit, submitted, timeLeft } = useTN3Quiz(userEmail);
  ...
  return (
    <div className="tn1-quiz-container">
      ...
      <h1 className="tn1-quiz-title">Bài Thi Trắc Nghiệm Quy Hoạch Tuyến Tính Chương 3</h1>
      ...
      {student.TN3 !== -1 ? (
        <div className="tn1-result-container">
          ...
          <p className="tn1-result-text">
            Điểm số của bạn: <strong>{student.TN3}</strong> / 10
          </p>
        </div>
      ) : (
