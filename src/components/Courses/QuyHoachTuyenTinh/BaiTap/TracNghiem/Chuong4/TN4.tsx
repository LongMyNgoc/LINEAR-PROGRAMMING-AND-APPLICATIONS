...
import { useTN4Quiz } from "./useTN4Quiz";

const TN4 = ({ setSelectedItem }: { setSelectedItem: (value: string | null) => void }) => {
  const storedUser = localStorage.getItem("user");
  const userEmail = storedUser ? JSON.parse(storedUser).email : null;

  const { student, answers, handleAnswerChange, calculateScore, handleSubmit, submitted, timeLeft } = useTN4Quiz(userEmail);
  ...
  return (
    <div className="tn1-quiz-container">
      ...
      <h1 className="tn1-quiz-title">Bài Thi Trắc Nghiệm Quy Hoạch Tuyến Tính Chương 4 Phần 1</h1>
      ...
      {student.TN4 !== -1 ? (
        <div className="tn1-result-container">
          ...
          <p className="tn1-result-text">
            Điểm số của bạn: <strong>{student.TN4}</strong> / 10
          </p>
        </div>
      ) : (
