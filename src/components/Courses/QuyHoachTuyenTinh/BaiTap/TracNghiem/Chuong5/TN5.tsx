...
import { useTN5Quiz } from "./useTN5Quiz";

const TN5 = ({ setSelectedItem }: { setSelectedItem: (value: string | null) => void }) => {
  const storedUser = localStorage.getItem("user");
  const userEmail = storedUser ? JSON.parse(storedUser).email : null;

  const { student, answers, handleAnswerChange, calculateScore, handleSubmit, submitted, timeLeft } = useTN5Quiz(userEmail);
  ...
  return (
    <div className="tn1-quiz-container">
      ...
      <h1 className="tn1-quiz-title">Bài Thi Trắc Nghiệm Quy Hoạch Tuyến Tính Chương 4 Phần 2</h1>
