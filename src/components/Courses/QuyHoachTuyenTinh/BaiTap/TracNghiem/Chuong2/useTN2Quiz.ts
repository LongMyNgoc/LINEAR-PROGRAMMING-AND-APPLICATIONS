...
export const useTN2Quiz = (userEmail: string | null) => {
  ...
  useEffect(() => {
    if (!userEmail || student?.TN2 !== -1) return;
    
    const storedStartTime = localStorage.getItem("tn2_start_time");
    let startTime = storedStartTime ? new Date(storedStartTime) : new Date();
    if (!storedStartTime) localStorage.setItem("tn2_start_time", startTime.toISOString());
    ...
    const updateTimer = () => {
      ...
      if (remainingTime === 0) {
        clearInterval(timer);
        handleSubmit();
        localStorage.removeItem("tn2_start_time");  // Xóa thời gian khi hết thời gian
      }
    };
  ...
  const handleSubmit = async () => {
    ...
    setSubmitted(true);
    try {
      await updateDoc(doc(db, "Students", userEmail), { TN2: calculateScore() });
    } catch (error) {
